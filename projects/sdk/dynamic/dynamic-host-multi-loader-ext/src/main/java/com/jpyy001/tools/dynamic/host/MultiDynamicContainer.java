...
public class MultiDynamicContainer {
     ...
     */
    public static boolean loadContainerApk(String containerKey, TargetPackage containerApk) {
        ...
        if (containerClassLoader != null) {
            ...
            if (mLogger.isInfoEnabled()) {
                mLogger.info("该containKey的apk已经加载过, containKey=" + containerKey +
                        ", last apkPath=" + apkFilePath + ", new apkPath=" + containerApk.targetFilePath);
            }

            if (TextUtils.equals(apkFilePath, containerApk.targetFilePath)) {
        ...
        try {
            ...
            if (mLogger.isInfoEnabled()) {
                mLogger.info("containerApk插入成功，containerKey=" + containerKey + ", path=" + containerApk.targetFilePath);
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    ...
    private static void hackContainerClassLoader(String containerKey, TargetPackage containerApk) throws Exception {
        ClassLoader pathClassLoader = MultiDynamicContainer.class.getClassLoader();
    ...
    private static class ContainerClassLoader extends BaseDexClassLoader {
        ...
        public ContainerClassLoader(String containerKey, TargetPackage targetPackage, ClassLoader parent) {
            super(targetPackage.targetFilePath, targetPackage.oDexPath != null ? new File(targetPackage.oDexPath) : null, targetPackage.libraryPath, parent);
            this.containerKey = containerKey;
            this.apkFilePath = targetPackage.targetFilePath;
        }
